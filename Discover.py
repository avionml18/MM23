...
if __name__ == "__main__":
    ...
    maze_1 = speedrun(0, 0, 0, maze_1)#call from starting square with curr-path 0
    directions = generate_directions(3, 3, maze_1)
    instructions = generate_instructions(3, 3, maze_1, directions)
    
    #prints shortest path of each square in the maze
    for i in range(DEFAULT_SIZE):
        #for j in range(4):
        for j in range(DEFAULT_SIZE):
            print(maze_1[j][i].ShortestPath, end=" | ")
        print()
